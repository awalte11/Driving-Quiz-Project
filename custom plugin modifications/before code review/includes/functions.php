...
add_action( 'wp_ajax_send_quiz_data', 'write_quiz_data' );
add_action( 'wp_ajax_nopriv_send_quiz_data', 'write_quiz_data' );

//function write_quiz_data($user_id, $score, $questions, $quiz_type)
function write_quiz_data()
{
   ...
   $wpdb->show_errors();
    ...
    $wpdb->insert(  $table_name, array( "user_id" => $user_id, "time" => date("Y-m-d H:i:s"), "score" => $score, "questions" => $questions, "quiz_type" => $quiz_type ) );
    wp_die();
}
...
function create_parent_child_link( $parent_id, $child_id)
{
    global $wpdb;
...
function remove_parent_child_link( $parent_id, $child_id)
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'parent_child_record_table';
    $extant = $wpdb->get_results( 
...
function create_mta_flag($user_id)
{
    global $wpdb;
...
function remove_mta_flag($user_id)
{
      global $wpdb;
    $table_name = $wpdb->prefix . 'mta_flag_table';
    $extant = $wpdb->get_results( 
...
function test_deploy_custom_records()
{
    create_parent_child_link(8, 2);
    create_mta_flag(8);
    
}

function delete_custom_user_records($user_id)
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'parent_child_record_table';

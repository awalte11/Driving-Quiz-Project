...
jQuery(".hdq_finsh_button").click(function() {
    ...
    function hdq_calculate_total(hdq_form_id) {
        ...
        hdq_custom_record_quiz(total_score, total_questions);//Single line added by ARW to call function to send quiz records to server
    ...
    //function sends quiz records to serve via ajaz
    //total score is correct answers, total questions is questions on quiz
    function hdq_custom_record_quiz(total_score, total_questions) {
            var userid = userdata.current_user_id;//gets user idea through enqueue/localize script wp functions over in functions.php
            ...
            if (userid != 0)
            {
                ...
                jQuery(document).ready(function($) {
                	var data = {
                		'action':'send_quiz_data',
                        'user_id': userid,
                        'score': total_score,
                        'questions': total_questions,
                        'quiz_type': hdq_quiz_name//doesn't need to be passed, something - not sure what - is making it available as can be seen in the stock functions below
                	};
                	
                	jQuery.post(wpa_data.admin_ajax, data, function(response) {
                		//doesn't need to do anything just send it.
                	});
                });
